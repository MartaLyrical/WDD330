...
export default class ToDos {
    ...
    addTodo() {
        ...
        if(elem.value == ''){
            errorMessage.innerHTML = "You cannot add an empty task! Plese enter a task"; 
        }else{
...
function renderTodoList(list, todos, element, hidden) {
    ...
    list.forEach(todo => {
        ...
        let btn = elem.childNodes[1]; 
        btn.addEventListener("click", function () {
            todos.removeTodo(todo.id)
        });

        let checkB = elem.childNodes[0].childNodes[0]; 
        if (checkB) {
        ...
        console.log("Watchin completed tasks");
        ...
        console.log("Watching active tasks");
...
function taksUpdated() {
    console.log("Checking number of taks left to do");
    let numberOfTasks = document.getElementById("tasks");
